...
**Pre-built binary** (no Rust required). v0.9.3 defaults to **`engine: fast`** — instant graph index, no ONNX warm at startup.
...
Restart your IDE so MCP picks up the new server. Re-run the installer to **update** — then `neuromesh -V` should show **v0.9.3**.
...
Example from a **650k-token monorepo** (release **v0.9.3**, default `engine: fast`):
...
| [Changelog](docs/CHANGELOG.md)         | What changed in v0.9.3                           |
