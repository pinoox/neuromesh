...
pub fn execute() -> Result<()> {
    ...
    if total_files == 0 {
        println!(
            "\n⚠️ No source code files found in {}. Add code files to run evaluation.",
            current_dir.display()
        );
        return Ok(());
    }
    ...
    let top_files: Vec<String> = file_summaries
        .iter()
        .map(|(p, _)| p.clone())
        .take(3)
        .collect();
    ...
    if let Some(f1) = top_files.first() {
        ...
        dynamic_tasks.push(format!(
            "Add form validation and state management in {}",
            f1
        ));
    }
    if let Some(f2) = top_files.get(1) {
        dynamic_tasks.push(format!(
            "Connect navigation and theme tokens across {} and {}",
            top_files[0], f2
        ));
    } else {
    ...
    dynamic_tasks
        .push("Implement end-to-end user workflow with error handling and toasts".to_string());

    println!("\n🧠 ===========================================================================");
    println!(
        "   NeuroMesh Project Evaluation Report — [{}]",
        project_name
    );
    ...
    for (path, tok) in file_summaries.iter().take(5) {
        println!(
            "    ├─ {:<35} : {:>6} tokens ({:.1}% of project)",
            path,
            tok,
            (*tok as f32 / total_corpus_tokens.max(1) as f32) * 100.0
        );
    }
    ...
    let avg_reduction =
        ((sum_raw_tokens - sum_nm_tokens) as f32 / sum_raw_tokens.max(1) as f32) * 100.0;
    let saved_per_task = (sum_raw_tokens - sum_nm_tokens) / dynamic_tasks.len().max(1);

    let raw_cost_100 = (sum_raw_tokens as f32 / dynamic_tasks.len() as f32 * 100.0 / 1_000_000.0)
        * cost_per_1m_sonnet;
    let nm_cost_100 = (sum_nm_tokens as f32 / dynamic_tasks.len() as f32 * 100.0 / 1_000_000.0)
        * cost_per_1m_sonnet;
    ...
    println!(
        "  • Tokens Saved Per Prompt     : ~{} tokens / prompt",
        saved_per_task
    );
    println!(
        "  • Signal-to-Noise Ratio Boost : {:.1}x cleaner context delivered to LLM",
        (100.0 / (100.0 - avg_reduction).max(1.0))
    );
    println!(
        "  • Hallucination Risk Factor   : REDUCED by {:.0}% (irrelevant code pruned)",
        avg_reduction * 0.9
    );
    println!();
    println!("💰 Projected Cost Comparison (Claude 3.5 Sonnet / GPT-4o):");
    println!(
        "  ├─ For 100 Prompts  : ${:.4} (Raw) ──▶ ${:.4} (NeuroMesh)  [You Save: ${:.4}]",
        raw_cost_100, nm_cost_100, saved_cost_100
    );
