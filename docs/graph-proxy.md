...
Default is **`native`** — built-in graph (`neuromesh index`, tiered retrieval). **v0.9.10 default `engine: fast`** uses native graph + lexical routing with no ONNX. Proxy is opt-in when CBM is already indexed in your IDE.
