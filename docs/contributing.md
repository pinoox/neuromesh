...
Rust and TypeScript already go through tree-sitter **queries** (`src/queries/*.scm`) via `LanguageSpec`. Python, Go, Java, Kotlin, PHP, C#, Dart, Swift, and Ruby use the same driver (regex/`GenericParser` fallback). Vue has a scoped regex extractor. C / C++ stay on `GenericParser`. A new language is a registry row plus queries or a fallback — not a new `match` arm in the engine.
