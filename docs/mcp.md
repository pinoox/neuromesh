...
Transport: **stdio JSON-RPC** (`neuromesh mcp <workspace>`). **v0.9.11 default:** **`engine: fast`** — graph index + query-side lexical expansion; pass the prompt only to `get_context_packet`. Opt in to **`hybrid`** / **`deep`** for bundled MiniLM embed-primary.
