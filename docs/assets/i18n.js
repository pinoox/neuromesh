...
window.NMI18n = (function () {
  ...
  const dict = {
    en: {
      ...
      'hero.badgeRelease': 'v0.9.0 — zero-embed fast engine · instant index',
      'hero.desc': 'Ask in plain language — Persian, Arabic, English, anything. NeuroMesh v0.9.0 defaults to a <strong>zero-embed graph engine</strong>: server-assisted concept expansion finds the right symbols on a live code graph and sends a <strong>folded evidence packet</strong> instead of three thousand-line file dumps. Opt in to <strong>hybrid</strong> for bundled MiniLM when you need embedding-primary recall. Works with Cursor, Claude, Codex, VS Code, and every MCP client.',
      ...
      'stats.sub': 'v0.9.0 · graph-first retrieval · up to ~97% fewer tokens vs full workspace dump',
      ...
      'engines.lead': 'v0.9.0 ships with <strong>engine: fast</strong> — instant graph index, no ONNX warm. Install, index, pass your prompt — done. Opt in to <strong>hybrid</strong> / <strong>deep</strong> for bundled MiniLM sidecar; optional CBM proxy for <code>get_context_packet</code>.',
      ...
      'engines.seedDesc': '<strong>Default: fast engine</strong> — graph + server-assisted keywords; prompt only. Opt-in: <code>hybrid</code> or <code>deep</code> via <code>neuromesh config engine</code>.',
    ...
    fa: {
      ...
      'hero.badgeRelease': 'نسخه 0.9.0 — موتور fast بدون embed · index فوری',
      'hero.desc': 'به هر زبانی بپرسید. NeuroMesh v0.9.0 پیش‌فرض <strong>موتور گراف بدون ONNX</strong> دارد: گسترش مفهومی سمت سرور symbol درست را روی گراف زنده پیدا می‌کند و <strong>بستهٔ تا‌شده</strong> می‌فرستد — نه dump هزار خطی. برای recall embed-primary، <strong>hybrid</strong> را فعال کنید. Cursor، Claude، VS Code و هر clientِ MCP.',
      ...
      'stats.sub': 'نسخه 0.9.0 · بازیابی graph-first · تا ~۹۷٪ توکن کمتر نسبت به dump کل workspace',
      ...
      'engines.lead': 'v0.9.0 با <strong>engine: fast</strong> می‌آید — index گراف فوری، بدون ONNX. نصب، index، prompt — تمام. برای sidecar MiniLM: <strong>hybrid</strong> / <strong>deep</strong>; CBM proxy اختیاری.',
      ...
      'engines.seedDesc': '<strong>پیش‌فرض: engine fast</strong> — گراف + keyword سمت سرور؛ فقط prompt. اختیاری: <code>hybrid</code> یا <code>deep</code> با <code>neuromesh config engine</code>.',
