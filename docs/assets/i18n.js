...
window.NMI18n = (function () {
  ...
  const dict = {
    en: {
      ...
      'how.lead': 'Intent → Concept → Graph, then fold. Tiered L1→L2→L3 only on critical gaps — seeds are never truncated to fake a small packet.',
      ...
      'stats.sub': 'Release 2026-08-30 v0.8.2 · 650,859 workspace tokens',
      'stats.s1': '% vs workspace (handle_tool_call)',
      'stats.s2': '% vs workspace (physarum)',
      'stats.s3': 'Files indexed',
      'stats.s4': 'ms packet activation',

      'engines.label': 'Configuration',
      'engines.title': 'Graph backend & seed engine',
      'engines.lead': 'Native graph by default (recommended). Optionally delegate get_context_packet to codebase-memory-mcp (CBM); v0.8.2 fixes proxy parsing, assisted forwarding, and honest metadata.',
      'engines.graphTitle': 'Graph backend',
      'engines.graphDesc': 'native · auto · proxy_cbm. Auto-detect reads ~/.cursor/mcp.json. Only get_context_packet uses the proxy; folding and trace stay native.',
      'engines.seedTitle': 'Seed engine',
      'engines.seedDesc': 'keywords_expanded (default), hybrid, semantic_lite. Controls keyword→symbol resolution before folding.',
      'engines.docsLink': 'Full engines guide →',
      ...
      'connect.lead': 'Native <strong>MCP stdio</strong> — Cursor, Claude, Codex, OpenCode, MiMo CLI, Antigravity, VS Code, Kilo Code, Trae, Windsurf, Cline/Roo, Zed, and more. Default config uses the portable command <code>neuromesh</code> (no absolute path) — workspace is auto-detected from the IDE.',
    ...
    fa: {
      ...
      'how.lead': 'Intent → Concept → Graph، بعد fold. L1→L2→L3 فقط روی شکاف‌های بحرانی — seedها برای کوچک‌نمایی بریده نمی‌شوند.',
      ...
      'stats.sub': 'انتشار 2026-08-30 v0.8.2 · 650,859 توکن workspace',
      ...
      'engines.label': 'پیکربندی',
      'engines.title': 'بک‌اند گراف و موتور بذر',
      'engines.lead': 'گراف داخلی پیش‌فرض (توصیه‌شده). در صورت نیاز فقط get_context_packet را به codebase-memory-mcp (CBM) بسپارید؛ v0.8.2 پروکسی را پایدار کرد (parse، assisted، metadata صادقانه).',
      'engines.graphTitle': 'بک‌اند گراف',
      'engines.graphDesc': 'native · auto · proxy_cbm. Auto از ~/.cursor/mcp.json می‌خواند. فقط get_context_packet از پروکسی استفاده می‌کند؛ folding و trace بومی می‌مانند.',
      'engines.seedTitle': 'موتور بذر',
      'engines.seedDesc': 'keywords_expanded (پیش‌فرض)، hybrid، semantic_lite. نگاشت keyword→symbol قبل از folding.',
      'engines.docsLink': 'راهنمای کامل موتورها →',
      ...
      'connect.title': 'اتصال',
      'connect.lead': '<strong>MCP stdio</strong> بومی — Cursor، Claude، Codex، OpenCode، MiMo CLI، Antigravity، VS Code، Kilo Code، Trae، Windsurf، Cline/Roo، Zed و بیشتر. پیکربندی پیش‌فرض فقط دستور <code>neuromesh</code> را می‌نویسد (بدون مسیر absolute) — workspace از IDE تشخیص داده می‌شود.',
  ...
  const pipelineSteps = {
    en: [
      { icon: '💬', title: 'Prompt', phase: 'read', key: 'pipe.0',
        desc: 'Normalize to a QueryPlan (routing, middleware, auth, …). Task text stays as written — handle_tool_call intent is not lowercased into mush.' },
      { icon: '🔍', title: 'Concepts + aliases', phase: 'read', key: 'pipe.1',
        desc: 'Static alias clusters map NL terms to concepts; the code-derived concept index turns concepts into symbol seeds — no embedding in L1/L2.' },
      { icon: '🕸️', title: 'Graph in RAM', phase: 'route', key: 'pipe.2',
        desc: 'L1 resolve on the neural mesh: files, functions, Imports, Calls. Edges exist only when the target is unique.' },
      { icon: '🌱', title: 'Seed files (L1)', phase: 'route', key: 'pipe.3',
        desc: 'Owners of resolved symbols always go in. Sufficiency is conservative — default claim is partial when uncertain.' },
      { icon: '🦠', title: 'Physarum + patterns', phase: 'route', key: 'pipe.4',
        desc: 'Slime mold connects seeds on a neighborhood subgraph. L2 pattern expand (max 6 files) only when critical gaps remain.' },
      { icon: '⚡', title: 'Escalate + fill', phase: 'splice', key: 'pipe.5',
        desc: 'Single-pass L1→L2→L3: L3 semantic recovery (max 2 seeds) is rare. Callees and synapses fill a real token budget.' },
      ...
      { icon: '📦', title: 'Evidence packet', phase: 'splice', key: 'pipe.7',
        desc: 'Ship the compact packet with optional retrieval metadata (level, claim, gaps). Wake folds with neuromesh_expand_fold — then record_feedback.' },
    ],
    fa: [
      { icon: '💬', title: 'Prompt', phase: 'read', key: 'pipe.0',
        desc: 'نرمال‌سازی به QueryPlan (routing، middleware، auth، …). متن task همان‌طور که نوشته شده می‌ماند.' },
      { icon: '🔍', title: 'Concepts + aliases', phase: 'read', key: 'pipe.1',
        desc: 'خوشه‌های alias اصطلاحات NL را به concept نگاشت می‌کنند؛ concept index مشتق از کد seed می‌سازد — بدون embedding در L1/L2.' },
      { icon: '🕸️', title: 'Graph in RAM', phase: 'route', key: 'pipe.2',
        desc: 'L1 روی مش عصبی resolve می‌کند: فایل، تابع، Imports، Calls. لبه فقط وقتی target یکتا باشد.' },
      { icon: '🌱', title: 'Seed files (L1)', phase: 'route', key: 'pipe.3',
        desc: 'فایل‌های صاحب symbol همیشه در بسته می‌آیند. sufficiency محافظه‌کارانه — claim پیش‌فرض partial وقتی نامطمئن.' },
      { icon: '🦠', title: 'Physarum + patterns', phase: 'route', key: 'pipe.4',
        desc: 'کپک مخاطی seedها را روی subgraph وصل می‌کند. L2 pattern expand (حداکثر ۶ فایل) فقط روی شکاف‌های بحرانی.' },
      { icon: '⚡', title: 'Escalate + fill', phase: 'splice', key: 'pipe.5',
        desc: 'L1→L2→L3 تک‌پاس: L3 semantic recovery (حداکثر ۲ seed) نادر است. callee و سیناپس بودجهٔ token واقعی را پر می‌کنند.' },
      ...
      { icon: '📦', title: 'Evidence packet', phase: 'splice', key: 'pipe.7',
        desc: 'بستهٔ فشرده با metadata اختیاری retrieval (level، claim، gaps). fold را با neuromesh_expand_fold بیدار کن — بعد record_feedback.' },
    ],
  };

  const pipeNodeKeys = {
    en: [
      { title: 'Prompt', key: 'QueryPlan intent' },
      { title: 'Concepts + aliases', key: 'NL → concept → symbol' },
      { title: 'Graph in RAM', key: 'Files · Calls · Imports' },
      { title: 'Seed files (L1)', key: 'conservative sufficiency' },
      { title: 'Physarum + patterns', key: 'L2 on critical gaps' },
      { title: 'Escalate + fill', key: 'single-pass L1→L3' },
      { title: 'Exon / intron', key: 'bodies → fold markers' },
      { title: 'Evidence packet', key: 'retrieval + expand_fold' },
    ],
    fa: [
