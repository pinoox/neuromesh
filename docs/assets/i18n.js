...
window.NMI18n = (function () {
  ...
  const dict = {
    en: {
      ...
      'hero.badgeRelease': 'v0.8.2 — ready to use',
      ...
      'stats.sub': 'v0.8.2 · up to ~97% fewer tokens vs dumping the whole workspace',
      ...
      'engines.lead': 'Start with the built-in graph — it is faster and more precise for daily coding. Optional: plug in an external code graph (CBM) for <code>get_context_packet</code> only; folding and trace stay local.',
      'engines.graphTitle': 'Where context comes from',
      'engines.graphDesc': '<code>native</code> (default) · <code>auto</code> · <code>proxy_cbm</code>. Auto picks up CBM if your IDE already has it. Change in Settings or one CLI command.',
      'engines.seedTitle': 'How prompts find symbols',
      'engines.seedDesc': '<code>keywords_expanded</code> (default) works great for mixed NL + code. Try <code>hybrid</code> for vague prompts without client keywords.',
      'engines.docsLink': 'Full setup guide →',

      'install.label': 'Get started',
      'install.title': 'Install NeuroMesh',
      'install.lead': 'One command, then <code>neuromesh doctor</code> to verify, and <code>neuromesh connect</code> to wire your editor.',
    ...
    fa: {
      ...
      'hero.badgeRelease': 'نسخه 0.8.2 — آماده استفاده',
      ...
      'stats.sub': 'نسخه 0.8.2 · تا ~۹۷٪ توکن کمتر نسبت به dump کل workspace',
      ...
      'engines.lead': 'با گراف داخلی شروع کنید — برای کار روزانه سریع‌تر و دقیق‌تر است. اختیاری: گراف خارجی (CBM) فقط برای <code>get_context_packet</code>؛ folding و trace محلی می‌مانند.',
      'engines.graphTitle': 'context از کجا بیاید',
      'engines.graphDesc': '<code>native</code> (پیش‌فرض) · <code>auto</code> · <code>proxy_cbm</code>. Auto اگر CBM در IDE دارید پیدا می‌کند.',
      'engines.seedTitle': 'prompt چطور symbol پیدا کند',
      'engines.seedDesc': '<code>keywords_expanded</code> (پیش‌فرض) برای NL + code عالی است. <code>hybrid</code> برای prompt مبهم بدون keyword.',
      'engines.docsLink': 'راهنمای کامل تنظیمات →',

      'install.label': 'شروع کن',
      'install.title': 'نصب NeuroMesh',
      'install.lead': 'یک دستور، بعد <code>neuromesh doctor</code> برای اطمینان، و <code>neuromesh connect</code> برای وصل کردن editor.',
      'install.tabMac': 'macOS / Linux',
      'install.tabWin': 'Windows',
