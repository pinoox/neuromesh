...
`tests/gold_tasks.toml` lists prompts and **path-qualified** gold files. Fixture repos live in `tests/fixtures/` (router, TS store, session, queue, string config, Python SMS, Kotlin SMS, Dart SMS, C# SMS), including edit/refactor-style prompts — not only “where is this symbol”.
