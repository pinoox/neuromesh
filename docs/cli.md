...
NeuroMesh v0.9.10 uses one **retrieval engine** preset: `fast` | `hybrid` | `deep`. Default **`fast`** — graph index + query-side lexical expansion; pass the prompt only.
...
`eval --release-gates` runs the v0.9.10 tiered holdout matrix per engine (`--engine fast|hybrid|deep`; recall ≥55%, precision ≥73%, no_seed ≤2).
